...
.. warning::
    ...
    When building an Apptainer, you are the user root.  This means that authentication is using your security keys (i.e. SSH) from your root account.  Keep this in mind when creating an Apptainer `.def` file.

.. warning::

    When shelling into an Apptainer, the user Id inside the container is different from the user Id outside the container.  Keep this in mind when executing commands that rely on the host system.
